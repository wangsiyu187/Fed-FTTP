...
class FeatureExtractor(nn.Module):
    """Wrapper that returns (features, logits) from a CNN backbone.
    Stores features on the final layer module to survive deepcopy."""
    ...
    def _register_hook(self):
        def hook_fn(module, input):
            module._extracted_features = input[0]

        if hasattr(self.backbone, 'fc'):
            self._final_layer = self.backbone.fc
        elif hasattr(self.backbone, '_fc'):
            self._final_layer = self.backbone._fc
        elif hasattr(self.backbone, 'classifier'):
            self._final_layer = self.backbone.classifier
        elif hasattr(self.backbone, 'last_linear'):
            self._final_layer = self.backbone.last_linear

        if self._final_layer is not None:
            self._final_layer.register_forward_pre_hook(hook_fn)

    def forward(self, x):
        logits = self.backbone(x)
        features = self._final_layer._extracted_features if self._final_layer is not None else None
