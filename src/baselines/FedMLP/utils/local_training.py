...
class LocalUpdate(object):
    def __init__(self, args, client_id, dataset, idxs, class_pos_idx, class_neg_idx, active_class_list=None, student=None, teacher_neg=None, teacher_act=None, dataset_test=None):
        ...
        self.loss_w = [len(self.local_dataset) / max(i, 1) for i in self.class_num_list]
        self.loss_w_unknown = [1] * len(self.class_num_list)
        self.loss_w_unknown[client_id] = len(self.local_dataset) / max(self.class_num_list[client_id], 1)
        ...
        if rnd < self.args.rounds_FedMLP_stage1:  # stage1
            ...
            for epoch in range(self.args.local_ep):
                ...
                for j, (samples, item, active_class_list) in enumerate(self.ldr_train):
                    ...
                    if self.args.annotation_num == self.args.n_classes:
                        # Full-label mode: loss over all classes, no negative/unsup split
                        loss_sup = loss_sup.sum() / (self.args.batch_size * self.args.n_classes)
                        loss = loss_sup
                    else:
                        # Original PSL logic
                        loss_sup = loss_sup[:, active_class_list_client].sum() / (
...
class DatasetSplit(Dataset):
    ...
    def __getitem__(self, item):
        ...
        # Full-label mode: no label zeroing — all classes active
