...
if __name__ == '__main__':
    ...
    # ------------------------------ dataset ------------------------------
    dataset_train, dataset_test, dataset_test_onsite, dict_users = get_dataset(args)
    ...
    if dataset_test is not None:
        logging.info(
            f"test (offsite): {np.sum(dataset_test.targets, axis=0)}, total: {len(dataset_test.targets)}")
    if dataset_test_onsite is not None:
        logging.info(
            f"test (onsite): {np.sum(dataset_test_onsite.targets, axis=0)}, total: {len(dataset_test_onsite.targets)}")
    ...
    if args.train:
        ...
        for i in user_id:
            active_cls = list(range(args.n_classes)) if args.dataset in ("ODIR", "ODIR_ATP") else [i]
            trainer_locals_1.append(LocalUpdate(
                args, i, deepcopy(dataset_train), dict_users[i], class_pos_idx_1, class_neg_idx_1, dataset_test=dataset_test, active_class_list=active_cls, student=deepcopy(netglob).to(args.device),
        ...
        for run in range(args.runs):
            ...
            for rnd in range(0, args.rounds_warmup):
                ...
                for i in tqdm(user_id):  # training over the subset
                    ...
                    if args.exp == 'FedMLP':
                        # Full-label mode: stay in stage1 always (no PSL distillation)
                        if args.dataset in ("ODIR", "ODIR_ATP"):
                            result = local.train_FedMLP(
                                rnd, tao, Prototype, writer1, negetive_class_list=[], active_class_list_client_i=list(range(args.n_classes)), net=deepcopy(netglob).to(args.device))
                        else:
                            result = local.train_FedMLP(
                                rnd, tao, Prototype, writer1, negetive_class_list=None, active_class_list_client_i=None, net=deepcopy(netglob).to(args.device))
                        w_local, loss_local, loss_false_negetive, loss_true_negetive, negetive_class_list_client, active_class_list_client = result[:6]
                    ...
                    # store every updated model (skip for ODIR/ODIR_ATP full-label mode)
                    if args.exp == 'FedMLP' and args.dataset not in ("ODIR", "ODIR_ATP") and rnd >= args.rounds_FedMLP_stage1-1:
                ...
                if args.exp == 'RSCFed':
                    ...
                    if args.dataset in ("ODIR", "ODIR_ATP"):
                        # Full-label mode: always FedAvg, no tao/Prototype
                        w_glob_fl = FedAvg(w_locals, dict_len)
                        netglob.load_state_dict(deepcopy(w_glob_fl))
                    elif rnd < args.rounds_FedMLP_stage1 - 1:
                        w_glob_fl = FedAvg(w_locals, dict_len)
                        netglob.load_state_dict(deepcopy(w_glob_fl))
                    else:
                        w_glob_fl = FedAvg(w_locals, dict_len)
                ...
                if rnd % 10 == 9:
                    logging.info(
                        "\n------------------------------> testing, run: %d, round: %d <------------------------------" % (
                            run, rnd))
                    for test_set_name, test_ds in [("Off-site", dataset_test), ("On-site", dataset_test_onsite)]:
                        if test_ds is None:
                            continue
                        result = globaltest(deepcopy(netglob).to(args.device), test_dataset=test_ds, args=args)
                        mAP, BACC, R, F1, auroc, P, hamming_loss, Kappa = result["mAP"], result["BACC"], result["R"], result[
                            "F1"], result["auc"], result["P"], result["hamming_loss"], result["Kappa"]
                        logging.info(
                            "[%s] -----> mAP: %.2f, BACC: %.2f, R: %.2f, F1: %.2f, auc: %.2f, P: %.2f, hamming_loss: %.2f, Kappa: %.2f" % (
                                test_set_name, mAP, BACC * 100, R * 100, F1 * 100, auroc * 100, P * 100, hamming_loss, Kappa * 100))
                        writer1.add_scalar(f'test_run{run}/{test_set_name}/mAP', mAP, rnd)
                        writer1.add_scalar(f'test_run{run}/{test_set_name}/BACC', BACC, rnd)
                        writer1.add_scalar(f'test_run{run}/{test_set_name}/F1', F1, rnd)
                        writer1.add_scalar(f'test_run{run}/{test_set_name}/auc', auroc, rnd)
                        writer1.add_scalar(f'test_run{run}/{test_set_name}/Kappa', Kappa, rnd)
                    logging.info('\n')
                if rnd == 49:
                ...
                if rnd % 10 == 9:
                    ...
                    for test_set_name, test_ds in [("Off-site", dataset_test), ("On-site", dataset_test_onsite)]:
                        if test_ds is None:
                            continue
                        result = globaltest(deepcopy(netglob).to(args.device), test_dataset=test_ds, args=args)
                        mAP, BACC, R, F1, auroc, P, hamming_loss, Kappa = result["mAP"], result["BACC"], result["R"], result["F1"], result["auc"], result["P"], result["hamming_loss"], result["Kappa"]
                        logging.info("[%s] -----> mAP: %.2f, BACC: %.2f, R: %.2f, F1: %.2f, auc: %.2f, P: %.2f, hamming_loss: %.2f, Kappa: %.2f"  % (test_set_name, mAP, BACC*100, R*100, F1*100, auroc*100, P*100, hamming_loss, Kappa*100))
                    ...
                    writer1.add_scalar(f'corr-test_run{run}/Kappa', Kappa, rnd)
                    logging.info('\n')
